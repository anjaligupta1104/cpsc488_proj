...
model_path = "llava/LLaVA-RLHF-7b-v1.5-224/sft_model"
lora_path = "llava/LLaVA-RLHF-7b-v1.5-224/rlhf_lora_adapter_model"
...
def get_labels_img_embeddings(images_dl: DataLoader[IATDataset], model: ClipLike,image_processor, device: torch.device,
    ...
    for batch in tqdm(images_dl, desc="Embedding images", disable=not progress):
        ...
        with torch.no_grad():
            ...
            image_embeddings.append(image_processor.preprocess(batch["img"].to(device)).cpu(), return_tensors='pt')['pixel_values'][0]).to(dtype=torch.bfloat16)
...
def measure_bias(cliplike: ClipLike, img_preproc: Callable, tokenizer: Callable, attribute="gender", opts=DEFAULT_OPTS):
    # do measurement
    ds = FairFace(mode="val", iat_type=attribute, transforms=None)
    ...
    #need to modify
    labels_list, image_embeddings = get_labels_img_embeddings(dl, cliplike,img_preproc, device, progress=True)
...
model, img_preproc, tokenizer, alias_name = model,image_processor,tokenizer,"debias_model_test"
model.eval()

# measure bias, lower == less biased
print(measure_bias(model, img_preproc, tokenizer, attribute="race"))